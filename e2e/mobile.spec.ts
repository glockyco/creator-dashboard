...
test('core pages remain usable on a narrow viewport', async ({ page }) => {
  ...
  await expect(primaryNav.getByRole('link', { name: /Dashboard/ })).toHaveAttribute('aria-current', 'page');
  ...
  primaryNav = await openDrawerNav(page);
  await expect(primaryNav.getByRole('link', { name: /Health/ })).toHaveAttribute('aria-current', 'page');
  await page.keyboard.press('Escape');

  await page.goto('/posts', { waitUntil: 'domcontentloaded' });
  await expect(page.getByRole('link', { name: 'Release notes' })).toBeVisible();
  primaryNav = await openDrawerNav(page);
  await expect(primaryNav.getByRole('link', { name: /Posts/ })).toHaveAttribute('aria-current', 'page');
  await page.keyboard.press('Escape');
  ...
  primaryNav = await openDrawerNav(page);
  await expect(primaryNav.getByRole('link', { name: /Timeline/ })).toHaveAttribute('aria-current', 'page');
  await page.keyboard.press('Escape');

  const chartScroll = await page.getByTestId('timeline-chart').evaluate((element) => ({
