...
import { buildProxyHeaders, parsePreviewLocalArgs, stripHopByHopHeaders, upstreamUrlFor } from './preview-local';
...
describe('preview-local helpers', () => {
  ...
  it('parses explicit local preview flags', () => {
    ...
    expect(() => parsePreviewLocalArgs(['--worker-port'])).toThrow('--worker-port requires a numeric value');
    expect(() => parsePreviewLocalArgs(['--unknown'])).toThrow('unknown argument: --unknown');
  });
