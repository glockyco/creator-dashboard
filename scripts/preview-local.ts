...
export function upstreamUrlFor(requestUrl: string | undefined, workerPort: number): URL {
  return new URL(requestUrl ?? '/', `http://${host}:${workerPort}`);
}

export function buildProxyHeaders(incomingHeaders: IncomingHttpHeaders, accessToken: string): Headers {
...
export function stripHopByHopHeaders(headers: Iterable<[string, string]>): [string, string][] {
...
export async function startAuthenticatedProxy(options: {
  ...
  const server = createServer((request, response) => {
    void proxyRequest(request, response, options).catch((error: unknown) => {
      ...
      response.writeHead(502, { 'Content-Type': 'text/plain' }).end(`local preview proxy failed: ${message}`);
    });
  });
  await new Promise<void>((resolve) => server.listen(options.proxyPort, host, resolve));
  return {
    server,
    stop: async () => {
...
export function createAccessTokenSigner(fixture: AccessFixture): AccessTokenSigner {
...
async function proxyRequest(
  ...
  const upstream = await fetch(upstreamUrlFor(request.url, options.workerPort), {
    method,
    headers: buildProxyHeaders(request.headers, await options.signAccessToken()),
    body: method === 'GET' || method === 'HEAD' ? undefined : request,
    duplex: method === 'GET' || method === 'HEAD' ? undefined : 'half',
    redirect: 'manual'
  } as RequestInit & { duplex?: 'half' });

  response.writeHead(upstream.status, upstream.statusText, stripHopByHopHeaders(upstream.headers.entries()));
...
async function main(): Promise<void> {
  ...
  try {
    await assertPortsAvailable([args.jwksPort, args.workerPort, args.proxyPort], host);
...
function parsePortValue(flag: string, value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port <= 0 || port > 65_535) throw new Error(`${flag} requires a numeric value`);
  return port;
}
