  ...
  const tone = $derived(
    status.last_status === 'success'
      ...
      : status.last_status === 'permanent_failure'
        ? 'danger'
        : status.last_status === 'transient_failure' || status.last_status === 'rate_limited_failure'
          ? 'warning'
          : 'muted'
  );
  const hasProblem = $derived(tone === 'danger' || tone === 'warning');
  const statusLabel = $derived(status.last_status ? status.last_status.replaceAll('_', ' ') : 'not run');
...
>
  ...
  {#if status.consecutive_failures > 0 || status.last_error}
    <div class="mb-5 rounded-xl border border-danger/30 bg-danger/10 px-3 py-2 text-sm text-danger">
      {status.consecutive_failures} consecutive {status.consecutive_failures === 1
        ? 'failure'
