...
<section class="overflow-hidden rounded-xl border border-border bg-bg-secondary">
  <div class="border-b border-border p-5">
    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-fg-muted">Library</p>
    <h2 class="mt-1 text-lg font-semibold">Posts</h2>
    <p class="text-sm text-fg-muted">Each row opens the post detail with source performance context.</p>
  </div>

  {#if posts.length === 0}
    <div class="p-5">
      <div class="rounded-lg border border-dashed border-border bg-bg-primary p-8 text-center">
        <h3 class="font-medium text-fg-primary">No posts match these filters</h3>
        <p class="mt-2 text-sm text-fg-muted">
          Clear the active author, tag, or source filter to recover the full content index.
        </p>
      </div>
    </div>
  {:else}
      ...
      <div class="divide-y divide-border">
        {#each posts as post (post.slug)}
          ...
          >
            ...
            <time class="text-sm text-fg-muted" datetime={new Date(post.posted_at).toISOString()}
      ...
      {#each posts as post (post.slug)}
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <time class="shrink-0 text-xs text-fg-muted" datetime={new Date(post.posted_at).toISOString()}
              >{formatPostedDate(post.posted_at)}</time
            >
          </div>
          {#if post.body_excerpt}
