...
<section class="rounded-xl border border-border bg-bg-secondary p-5">
  ...
  <div class="space-y-3">
    {#each posts as post (post.slug)}
      <!-- eslint-disable svelte/no-navigation-without-resolve -- post.url is upstream content (external) -->
      <a
        class="block rounded-lg border border-border bg-bg-primary p-4 hover:border-fg-muted"
        href={post.url}
        target="_blank"
        rel="noreferrer"
      >
