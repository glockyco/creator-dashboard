<script lang="ts">
  ...
  const metricPoints = $derived(
    timeline.metricSeries.flatMap((series) =>
      series.points.map((point) => ({
        ...
        label: `${sourceName(timeline, series.source_id)} · ${series.metric.replaceAll('_', ' ')}`
      }))
    )
  );
  const eventMarkers = $derived(
    overlays.includes('events')
      ? timeline.events.map((event) => ({ date: new Date(event.ts), title: event.title ?? event.kind }))
      : []
  );
  const postMarkers = $derived(
    overlays.includes('posts')
      ? timeline.posts.map((post) => ({ date: new Date(post.posted_at), title: post.title }))
      : []
  );
  ...
  async function renderPlot(
    version: number,
    points: typeof metricPoints,
    events: typeof eventMarkers,
    posts: typeof postMarkers
  ) {
...
<section class="rounded-xl border border-border bg-bg-secondary p-4 sm:p-5">
  <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex flex-wrap gap-3 text-xs text-fg-muted">
      <span data-testid="timeline-event-marker" class="inline-flex items-center gap-2"
        ><span class="h-4 w-0.5 bg-event"></span>Events ({eventMarkers.length})</span
      >
      <span data-testid="timeline-post-marker" class="inline-flex items-center gap-2"
        ><span class="h-4 w-0.5 border-l-2 border-dashed border-post"></span>Posts ({postMarkers.length})</span
      >
    </div>
  </div>
