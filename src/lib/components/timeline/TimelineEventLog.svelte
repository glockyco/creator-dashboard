    ...
    {#each items as item (`${item.type}-${item.source_id}-${item.ts}-${item.title}`)}
      <li class="rounded-lg border border-border bg-bg-primary p-4">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p class="text-xs uppercase tracking-wide {item.type === 'event' ? 'text-event' : 'text-post'}">
              {item.type} · {sourceName(timeline, item.source_id)}
            </p>
          ...
          <time class="shrink-0 text-sm text-fg-muted" datetime={new Date(item.ts).toISOString()}
            >{formatTimelineDate(item.ts)}</time
          >
        </div>
      </li>
    {:else}
      <li class="rounded-lg border border-dashed border-border p-6 text-center text-fg-muted">
        No posts or events match this timeline filter.
      </li>
    {/each}
  </ol>
</section>
