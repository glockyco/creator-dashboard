...
describe('fetchBingWebmaster', () => {
  it('emits aggregate and query/page breakdown metrics', async () => {
    ...
    expect(
      out.metric_points.find(
        (point) => point.dimensions?.query === 'ancient kingdoms' && point.metric === 'impressions'
      )?.value
    ).toBe(70);
  });
  ...
  it('throws ZodError on schema drift', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(new Response(JSON.stringify({ QueryStats: [{ Query: 'x' }] }), { status: 200 }))
    );
