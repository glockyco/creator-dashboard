...
describe('fetchGithub', () => {
  it('emits scalar, contribution, and repo star metrics without events', async () => {
    ...
    expect(out.metric_points.find((point) => point.metric === 'repo_stars')?.dimensions).toEqual({ repo: 'personal-website', archived: 'false' });
    expect(out.events).toEqual([]);
  });

  it('throws ZodError on schema drift', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ data: { viewer: null } }), { status: 200 })));
