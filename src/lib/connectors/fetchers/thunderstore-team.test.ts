...
const source = { id: 'thunderstore-wowmuch', name: 'Thunderstore: WoW_Much', identity: 'WoW_Much', category: 'platform', cadenceHours: 1, fetcher: fetchThunderstoreTeam, config: { namespace: 'WoW_Much' } } as const;
const env = {} as Env;
...
describe('fetchThunderstoreTeam', () => {
  it('emits total downloads, package count, and per-package downloads', async () => {
    ...
    expect(out.metric_points.find((point) => point.metric === 'package_downloads')?.dimensions).toEqual({ package: 'MapPins' });
