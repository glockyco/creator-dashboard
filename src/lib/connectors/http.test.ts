...
describe('fetchJson', () => {
  it('returns schema-parsed JSON for a 2xx response', async () => {
    ...
    await expect(fetchJson('https://example.invalid/api', { schema: z.object({ count: z.number() }) })).resolves.toEqual({ count: 3 });
  });

  it('throws FetchError for non-2xx responses with status and headers', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('bad token', { status: 401, headers: { 'Retry-After': '30' } })));
