...
function digestEnv(options: { alreadySent?: boolean } = {}) {
  ...
  const env = {
    ...
    DISCORD_DIGEST_WEBHOOK: 'https://discord.test/webhook'
  } as Env;
  return { env, calls };
}
...
function rowsFor(sql: string) {
  if (sql.includes('FROM metric_points'))
    return [{ source_id: 'github-glockyco', metric: 'followers', ts: 1, value: 13, dimensions: null }];
...
describe('maybeDailyDigest', () => {
  it('skips outside the Vienna digest hour before touching D1 or Discord', async () => {
    ...
    await expect(maybeDailyDigest(env, new Date('2026-01-15T04:00:00.000Z'))).resolves.toEqual({
      sent: false,
  ...
  it('posts the digest once per Vienna date and records it only after Discord succeeds', async () => {
    ...
    await expect(maybeDailyDigest(env, new Date('2026-01-15T05:00:00.000Z'))).resolves.toEqual({
      sent: true,
      reason: 'sent'
    });
    ...
    expect(JSON.parse(String(init.body))).toMatchObject({
      content: 'Creator Dashboard daily digest — 2026-01-15 — last 24h'
    });
