...
describe('consumeDlqMessage', () => {
  it('records DLQ failure, alerts once, and acks', async () => {
    ...
    const msg = { body: { source_id: 'source-a', dispatch_ts: 1, force: false }, ack: vi.fn() } as unknown as Message<{ source_id: string; dispatch_ts: number; force: boolean }>;

    await consumeDlqMessage(msg, env, 1714838400000);

    expect(prepare).toHaveBeenCalledWith('INSERT INTO fetcher_failures (source_id, ts, tier, status_code, error_message) VALUES (?, ?, ?, ?, ?)');
