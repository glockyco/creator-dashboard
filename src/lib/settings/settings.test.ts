...
describe('settings schema and store', () => {
  it('falls back to defaults for invalid stored settings', () => {
    expect(parseSettings({ theme: 'neon', defaultDateRange: '1d', identityColors: { glockyco: 'blue', WoW_Much: '#f59e0b' } })).toEqual(DEFAULT_SETTINGS);
  });
  ...
  it('persists valid settings and applies theme/color properties to the root element', () => {
    ...
    expect(store.current).toEqual({ theme: 'light', defaultDateRange: '7d', identityColors: { glockyco: '#111111', WoW_Much: '#222222' } });
