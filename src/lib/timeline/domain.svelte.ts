...
export type TimelineLogItem =
  | { type: 'event'; ts: number; source_id: string; title: string; subtitle: string | null; url: string | null; event: TimelineEvent }
  | { type: 'post'; ts: number; source_id: string; title: string; subtitle: string | null; url: string; post: TimelinePost };

export function timelineLog(events: TimelineEvent[], posts: TimelinePost[], overlays: TimelineOverlay[]): TimelineLogItem[] {
  const items: TimelineLogItem[] = [];
  if (overlays.includes('events')) {
    for (const event of events) {
      items.push({ type: 'event', ts: event.ts, source_id: event.source_id, title: event.title ?? event.kind, subtitle: event.body, url: event.url, event });
    }
  }
  if (overlays.includes('posts')) {
    for (const post of posts) {
      items.push({ type: 'post', ts: post.posted_at, source_id: post.source_id, title: post.title, subtitle: post.author, url: post.url, post });
    }
  }
