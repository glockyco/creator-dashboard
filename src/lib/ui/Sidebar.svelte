...
<script lang="ts">
  ...
  function toggleCollapsed() {
    ...
    localStorage.setItem('creator-dashboard-sidebar-collapsed', String(isCollapsed));
  }
  ...
  <Drawer.Portal>
    ...
    >
      <div class="flex h-14 shrink-0 items-center justify-between border-b border-border px-4">
        <Drawer.Title class="text-[0.65rem] font-semibold uppercase tracking-[0.28em] text-fg-muted">
          Creator Dashboard
        </Drawer.Title>
        <Drawer.Description class="sr-only">Primary navigation</Drawer.Description>
      </div>
...
<nav
  ...
  style="width: {isCollapsed ? SIDEBAR_WIDTH_COLLAPSED : SIDEBAR_WIDTH_EXPANDED}px"
  aria-label="Primary"
>
  <div
  ...
  >
    {#if !isCollapsed}
      <span class="truncate text-[0.65rem] font-semibold uppercase tracking-[0.28em] text-fg-muted"
  ...
  <ul class={`flex flex-1 flex-col gap-1 py-3 ${isCollapsed ? 'items-center px-2' : 'px-3'}`}>
    {#each items as item (item.href)}
      {@const Icon = item.icon}
      {@const active = isActive(item, page.url.pathname)}
