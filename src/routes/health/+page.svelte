...
<section class="space-y-6">
  ...
  >
    <div>
      <p class="text-sm uppercase tracking-wide text-fg-muted">
        Operations health
      </p>
      <h1 class="mt-2 text-3xl font-semibold">Fetcher status</h1>
      <p class="mt-2 max-w-2xl text-sm text-fg-muted">
        Review source freshness, recent ingestion failures, and alert delivery
        from one recovery surface.
      </p>
    </div>
    <div
      class="rounded-lg border border-border bg-bg-primary px-3 py-2 text-sm text-fg-muted"
    >
  ...
  <section class="rounded-xl border border-border bg-bg-secondary p-5">
    <div class="mb-4">
      ...
      <p class="text-sm text-fg-muted">
        Use consecutive failures and last success to decide what needs manual
        recovery.
      </p>
    </div>
    {#if data.runs.length === 0}
      <div
        class="rounded-lg border border-dashed border-border bg-bg-primary p-6 text-center text-sm text-fg-muted"
      >
        No fetcher runs have been recorded yet. Trigger a refresh, then return
        here to confirm source health.
      </div>
    {:else}
      <div class="overflow-x-auto">
        <table class="w-full min-w-[760px] text-left text-sm">
          <thead
            class="border-b border-border text-xs uppercase tracking-wide text-fg-muted"
          >
            <tr
              ><th class="px-3 py-3">Source</th><th class="px-3 py-3">Status</th
              ><th class="px-3 py-3">Last run</th><th class="px-3 py-3"
                >Last success</th
              ><th class="px-3 py-3">Failures</th><th class="px-3 py-3"
                >Latest error</th
              ></tr
            >
          </thead>
          <tbody class="divide-y divide-border">
            {#each data.runs as run}
              <tr class="align-top">
                <td class="px-3 py-3 font-mono text-fg-primary"
                  >{run.source_id}</td
                >
                <td class="px-3 py-3"
                  ><span
                  ...
                  ></td
                >
                <td class="px-3 py-3 text-fg-muted"
                  >{formatTimestamp(run.last_run_at)}</td
                >
                <td class="px-3 py-3 text-fg-muted"
                  >{formatTimestamp(run.last_success_at)}</td
                >
                <td class="px-3 py-3 font-medium">{run.consecutive_failures}</td
                >
                <td class="max-w-sm px-3 py-3 text-fg-muted"
                  >{run.last_error ?? "—"}</td
                >
              </tr>
            {/each}
  ...
  <div class="grid gap-4 xl:grid-cols-[2fr_1fr]">
    <section class="rounded-xl border border-border bg-bg-secondary p-5">
      <div class="mb-4">
        ...
        <p class="text-sm text-fg-muted">
          Newest failed ingest attempts with source, tier, HTTP status, and
          captured error.
        </p>
      </div>
      {#if data.failures.length === 0}
        <div
          class="rounded-lg border border-dashed border-border bg-bg-primary p-6 text-center text-sm text-fg-muted"
        >
          No recent failures. Current ingestion checks have not produced
          recovery work.
        </div>
      {:else}
        <div class="space-y-3">
          {#each data.failures as failure}
            <article class="rounded-lg border border-border bg-bg-primary p-4">
              ...
              >
                ...
                <time class="text-sm text-fg-muted"
                  >{formatTimestamp(failure.ts)}</time
                >
              </div>
    ...
    <section class="rounded-xl border border-border bg-bg-secondary p-5">
      <div class="mb-4">
        ...
        <p class="text-sm text-fg-muted">
          Recent alert keys and delivery timestamps.
        </p>
      </div>
