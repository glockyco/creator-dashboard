...
<section class="mx-auto max-w-3xl space-y-6">
  <div class="rounded-xl border border-border bg-bg-secondary p-5">
    ...
    <p class="mt-2 max-w-2xl text-sm text-fg-muted">
      Tune this browser's creator dashboard defaults. These client-only
      preferences do not write to D1.
    </p>
  </div>

  <section
    class="space-y-4 rounded-xl border border-border bg-bg-secondary p-5"
  >
    <div>
      <p
    ...
    {:else}
      <div
        class="rounded-lg border border-dashed border-border bg-bg-primary p-6 text-sm text-fg-muted"
        role="status"
      >
