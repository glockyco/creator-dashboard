  ...
  >
    <div>
      ...
      <p class="mt-2 max-w-2xl text-sm text-fg-muted">
        Correlate source metrics, content events, and posts across the selected
        window.
      </p>
    </div>
    <div
    ...
    >
      {data.timeline.sources.length} sources · {data.timeline.metricSeries
        .length} metric series
    </div>
  </div>

  <section
    class="space-y-3 rounded-xl border border-border bg-bg-secondary p-5"
  >
    <div>
      <p
      ...
      <p class="text-sm text-fg-muted">
        Adjust the date range, source IDs, and overlays before reading the chart
        and log.
      </p>
    </div>
  ...
  <div class="space-y-4">
    ...
    {#if data.timeline.metricSeries.length > 0}
      <TimelineChart
        ...
        overlays={data.filters.overlays}
      />
    {:else}
      <section
        class="rounded-xl border border-dashed border-border bg-bg-secondary p-8 text-center"
      >
        <h3 class="font-medium text-fg-primary">
          No metric points in this investigation window
        </h3>
        <p class="mt-2 text-sm text-fg-muted">
          Widen the date range or add source IDs to recover trend evidence
